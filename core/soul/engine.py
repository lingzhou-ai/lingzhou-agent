...
class SoulEngine:
    ...
    async def build_content(self, ethos_values: EthosValues | None = None) -> str:
        ...
        values = ethos_values or await self._persona.ethos_values() or EthosValues()
        constitution_hash = get_constitution_hash() or "unloaded"
        return self._render(soul_name, values, constitution_hash=constitution_hash)
    ...
    async def sync_md(self) -> None:
        ...
        if values is None:
            return
        soul_path = self._cfg.workspace_dir / "SOUL.md"
        soul_path.write_text(await self.build_content(values), encoding="utf-8")
