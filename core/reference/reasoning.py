...
async def reason_about_candidates_with_llm(
    ...
    user_content = f'用户消息："{message[:500]}"\n\n候选节点：\n{cand_block}'
...
async def reason_about_speaker_with_llm(
    ...
    *,
    ...
    for turn in (recent_turns or [])[-4:]:
        ...
        content = normalize_text(str(turn.get("content") or ""))[:200]
        if content:
            turns_block.append(f"- {role}: {content}")

    user_content = "\n".join(
        [
            f'当前用户消息："{message[:500]}"',
            f"当前 chat 句柄线索：{chat_id or '（无）'}",
            f"来源路由线索：{source_hint or '（无）'}",
            "从当前消息提取的线索：",
            f"- names: {cues.get('names', [])[:5]}",
            f"- preferences: {cues.get('preferences', [])[:3]}",
            f"- explicit: {cues.get('explicit', [])[:3]}",
            f"- source_traits: {cues.get('source_traits', [])[:3]}",
            "最近交互片段：",
            "\n".join(turns_block) if turns_block else "（无）",
            "当前 chat 连续性：",
            (chat_continuity or "（无）")[:400],
            "当前对象跨 chat 交互连续性：",
            (interlocutor_continuity or "（无）")[:400],
            "候选交互对象画像：",
