...
- **WM 中出现 `[已告知用户]` 条目时的回复原则**：
  - 这代表上一条消息已经发送给用户，内容在条目里。生成 `reply_to_user` 时**不要重复**已告知内容中的相同事实或结论；
  - 若本轮需要回复，仅补充新进展或回应用户的新问题；避免套用相同措辞（如"成本策略已记录""向量数据库是主力"等背景信息在同一对话中重复出现超过一次）。
  - **不要在每条回复里都重申背景**（任务数量、记忆结构、系统状态等），除非用户明确询问。
