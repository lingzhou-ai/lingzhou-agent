...
### `core/loop/drive/engine.py` — 自驱力引擎 (SelfDriveEngine)
...
### `core/evolution/` — 进化引擎 (EvolutionEngine)
...
### `core/plugin/` — 插件系统 (PluginManager)
...
### 任务存储 (TaskStore)
`store/task/` 是 SQLite 持久化主线，覆盖 tasks / chat_messages / failures / facts / signals / runs / meta_reflections。`memory/` 只保留工作记忆和少量上层 façade，不承载底层 schema 逻辑。
...
| ID | 蓝图要求 | 当前状态 | 偏差评估 |
|----|----------|----------|----------|
| P0-1 | 自动多模态感知闭环 | `tools/image.py`、`core/execution/workers.py` 与相关测试已落地；`core/perception/` 增加多模态入口提取，将用户输入中的图片标记纳入 perception 供 judgment 侧使用 | 已补齐第一阶段（感知入口）：多模态 marker 可直接进入 perception；剩余为“内容级别理解”到“跨模型归一化”链路 |
| P0-2 | Task-level model routing 自动闭环 | `task.model_tier`、`_prefer_tier_for_task()`、`_apply_tick_model_strategy()` 已存在，task 已能持久化 tier 并参与后续路由 | 已实现：`meta_reflection routing` 路径可在消费阶段将 `task.model_tier` 落写到任务状态；`reasoner/repair/reader` 均在白名单中，识别失败时保留为手工提示 |
...
| ID | 蓝图要求 | 当前状态 | 偏差评估 |
|----|----------|----------|----------|
| P2-1 | 进化效果验证（before-after 对比） | `core/evolution/` 已有 smoke / rollback 护栏，但未见统一的 before-after 结构化评分闭环 | 部分实现 |

### 小结

已经落地、因此不应再列为 gap 的能力包括：自主内环、Worker executors、Run 状态回流 Task、progress crystal、多 worker / 多任务并发，以及 MetaReflection 基础设施。
当前更准确的未完成点是：Run 抽象的进一步成熟、进化 before/after 结构化评估，以及多模态内容级理解与证据归一化链路（当前尚未成为持续常驻流程）。
